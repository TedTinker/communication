...
class Agent:
    ...
    def epoch(self, batch_size):
        """
        Run one epoch: train the forward model, critics, actor, and alpha values.
        Logs loss, reward, and curiosity metrics.
        """
        start_time = duration()
        self.epochs += 1
        self.total_epochs += 1
        ...
        # Train forward model
        hps, hqs, vision_is, touch_is, prop_is, command_voice_is, feedback_voice_is, pred_obs_p, pred_obs_q, labels = self.forward(
            torch.zeros((episodes, 1, self.args.pvrnn_mtrnn_size)), obs, actions
        )
        ...
        touch_loss = F.mse_loss(pred_obs_q.touch, touch[:, 1:], reduction='none').mean(-1).unsqueeze(-1) * mask * self.args.touch_scaler
        prop_loss = F.mse_loss(pred_obs_q.prop, prop[:, 1:], reduction='none').mean(-1).unsqueeze(-1) * mask * self.args.prop_scaler
        ...
        command_voice_loss = compute_individual_voice_loss(command_voice, pred_obs_q.command_voice, self.args.command_voice_scaler)
        feedback_voice_loss = compute_individual_voice_loss(feedback_voice, pred_obs_q.feedback_voice, self.args.feedback_voice_scaler)

        accuracy = (vision_loss + touch_loss + prop_loss + command_voice_loss + feedback_voice_loss).mean()

        # KL divergence (complexity) losses
        ...
        prediction_error_curiosity = (
            vision_prediction_error_curiosity + touch_prediction_error_curiosity +
            command_voice_prediction_error_curiosity + feedback_voice_prediction_error_curiosity
        )

        vision_hidden_state_curiosity = self.args.hidden_state_eta_vision * torch.clamp(vision_complexity, 0, self.args.dkl_max)
        touch_hidden_state_curiosity = self.args.hidden_state_eta_touch * torch.clamp(touch_complexity, 0, self.args.dkl_max)
        prop_hidden_state_curiosity = self.args.hidden_state_eta_prop * torch.clamp(prop_complexity, 0, self.args.dkl_max)
        command_voice_hidden_state_curiosity = self.args.hidden_state_eta_command_voice * torch.clamp(command_voice_complexity, 0, self.args.dkl_max)
        feedback_voice_hidden_state_curiosity = self.args.hidden_state_eta_feedback_voice * torch.clamp(feedback_voice_complexity, 0, self.args.dkl_max) * self.hidden_state_eta_feedback_voice_reduction

        hidden_state_curiosity = (
            vision_hidden_state_curiosity + touch_hidden_state_curiosity +
            prop_hidden_state_curiosity + command_voice_hidden_state_curiosity +
            feedback_voice_hidden_state_curiosity
        )
        ...
        extrinsic = reward.mean().item()
        intrinsic_curiosity = curiosity.mean().item()
        reward += curiosity

        # Critic training
        with torch.no_grad():
            new_action, log_pis_next, log_pis_next_text = self.actor(hqs.detach(), parenting)
            Q_target_nexts = [
                self.critic_targets[i](new_action, hqs.detach())
                for i in range(self.args.critics)
            ]
            log_pis_next = log_pis_next[:, 1:]
            log_pis_next_text = log_pis_next_text[:, 1:]

            Q_target_nexts = torch.stack(Q_target_nexts, dim=0)
            Q_target_next, _ = torch.min(Q_target_nexts, dim=0)
            Q_target_next = Q_target_next[:, 1:]

            alpha = self.alpha if self.args.alpha is None else self.args.alpha
            alpha_text = self.alpha_text if self.args.alpha_text is None else self.args.alpha_text
        ...
        # Actor training (periodic)
        if self.epochs % self.args.d == 0:
            alpha = self.alpha if self.args.alpha is None else self.args.alpha
            alpha_text = self.alpha_text if self.args.alpha_text is None else self.args.alpha_text

            new_action, log_pis, log_pis_text = self.actor(hqs[:, :-1].detach(), parenting)
            loc = torch.zeros(self.args.wheels_joints_shape, dtype=torch.float32).to(self.args.device)
            scale_tril = torch.eye(self.args.wheels_joints_shape)
            policy_prior = MultivariateNormal(loc=loc, scale_tril=scale_tril)
            policy_prior_log_prob = self.args.normal_alpha * policy_prior.log_prob(new_action.wheels_joints).unsqueeze(-1)

            intrinsic_entropy = ((alpha * log_pis - policy_prior_log_prob) * mask).mean().item()

            Qs = [self.critics[i](new_action, hqs[:, :-1].detach()) for i in range(self.args.critics)]
            Qs = torch.stack(Qs, dim=0)
            Q, _ = torch.min(Qs, dim=0)
            Q = Q.mean(-1).unsqueeze(-1)

            actor_loss = ((alpha * log_pis - policy_prior_log_prob) + (alpha_text * log_pis_text) - Q) * mask
        ...
        if self.args.alpha_text is None:
        ...
        if self.epochs == 1 or self.epochs >= self.args.epochs or self.epochs % self.args.keep_data == 0:
            def safe_mean(x): return x.mean().item() if x is not None else None

            self.plot_dict['accuracy_loss'].append(safe_mean(accuracy))
            self.plot_dict['vision_loss'].append(safe_mean(vision_loss))
            self.plot_dict['touch_loss'].append(safe_mean(touch_loss))
            self.plot_dict['prop_loss'].append(safe_mean(prop_loss))
            self.plot_dict['command_voice_loss'].append(safe_mean(command_voice_loss))
            self.plot_dict['feedback_voice_loss'].append(safe_mean(feedback_voice_loss))
            self.plot_dict['complexity_loss'].append(complexity.item() if complexity is not None else None)
            self.plot_dict['alpha_loss'].append(alpha_loss if alpha_loss is not None else None)
            self.plot_dict['alpha_text_loss'].append(alpha_text_loss if alpha_text_loss is not None else None)
            self.plot_dict['actor_loss'].append(actor_loss if actor_loss is not None else None)

            for i, loss in enumerate(critic_losses):
                value = log(loss.item()) if loss.item() > 0 else loss.item()
                self.plot_dict['critics_loss'][i].append(value)

            self.plot_dict['critics_loss'].append(critic_losses)
            self.plot_dict['extrinsic'].append(extrinsic)
            self.plot_dict['q'].append(-Q if Q is not None else None)
            self.plot_dict['intrinsic_curiosity'].append(intrinsic_curiosity)
            self.plot_dict['intrinsic_entropy'].append(intrinsic_entropy)

            self.plot_dict['vision_prediction_error_curiosity'].append(vision_prediction_error_curiosity.mean().item())
            self.plot_dict['touch_prediction_error_curiosity'].append(touch_prediction_error_curiosity.mean().item())
            self.plot_dict['prop_prediction_error_curiosity'].append(prop_prediction_error_curiosity.mean().item())
            self.plot_dict['command_voice_prediction_error_curiosity'].append(command_voice_prediction_error_curiosity.mean().item())
            self.plot_dict['feedback_voice_prediction_error_curiosity'].append(feedback_voice_prediction_error_curiosity.mean().item())

            self.plot_dict['vision_hidden_state_curiosity'].append(vision_hidden_state_curiosity.mean().item())
            self.plot_dict['touch_hidden_state_curiosity'].append(touch_hidden_state_curiosity.mean().item())
            self.plot_dict['prop_hidden_state_curiosity'].append(prop_hidden_state_curiosity.mean().item())
            self.plot_dict['command_voice_hidden_state_curiosity'].append(command_voice_hidden_state_curiosity.mean().item())
            self.plot_dict['feedback_voice_hidden_state_curiosity'].append(feedback_voice_hidden_state_curiosity.mean().item())

            self.plot_dict['prediction_error_curiosity'].append(prediction_error_curiosity.mean().item())
            self.plot_dict['hidden_state_curiosity'].append(hidden_state_curiosity.mean().item())
