...
print("\n\nLoading...", end = " ")
with open(f'saved/{hyper_parameters}/plot_dict.pickle', 'rb') as file:
...
# I get the impression this part isn't working?
...
# This part, though, I think it is.
these_parameters = agent_lists[f"{agent_num}_{epochs}"]
agent.load_state_dict(state_dict = these_parameters)



agent.tasks = {"0" : Task(actions = [1], objects = 2, colors = [0, 1, 2, 3, 4, 5], shapes = [0,], parent = True, args = agent.args)}
