...
class RecurrentReplayBuffer:
    ...
    def __init__(
        ...
        self.gc = np.zeros((self.args.capacity, self.max_episode_len + 1, args.objects * (len(shapes) + len(colors) + len(goals))), dtype='float32')
    ...
    def sample(self, batch_size):
        ...
        if self.segment_len is None:
            ...
            gc = as_tensor_on_device(gc).view(batch_size, max_ep_len_in_batch+1, self.args.objects * (len(shapes) + len(colors) + len(goals)))
            ...
            gc_seg = np.zeros((batch_size, self.segment_len + 1, self.args.objects * (len(shapes) + len(colors) + len(goals))))
