...
class Arena:
    ...
    def begin(self, objects, goal, parenting, set_positions=None):
        """
        Begin an episode by setting up the robot and placing objects.
        Handles goal exceptions and resets relevant episode-specific states.
        """
        self.set_pos()
        self.set_yaw()
        ...
        joint_angles = {
            joint_num: (getattr(self.args, f'max_joint_{joint_num}_angle') + 
                        getattr(self.args, f'min_joint_{joint_num}_angle')) / 2
            for joint_num in self.joint_indices.keys()
        }
        ...
        self.objects_in_play = {}
        self.durations = {key: {} for key in ['watch', 'be_near', 'top', 'push', 'left', 'right', 'except']}
        already_in_play = {key: 0 for key in shape_map.keys()}

        if set_positions is None:
        ...
        for i, (color, shape) in enumerate(objects):
            ...
            x, y = set_positions[i]
            p.resetBasePositionAndOrientation(object_index, (x, y, object_upper_starting_pos),
                                              (0, 0, 0, 1), physicsClientId=self.physicsClient)
            ...
            for task in self.durations.keys():
    ...
    def step(self, left_wheel_speed, right_wheel_speed, joint_target_velocities, verbose=False, sleep_time=None, waiting=False):
        """
        Advance simulation by one step (composed of multiple substeps).
        Controls wheel and joint motion with smooth interpolation.
        Tracks contacts and relative positions.
        """
        _, _, _, _, yaw = self.get_pos_spe_rpy(self.robot_index)
        self.robot_start_yaw = yaw
        self.objects_start = self.get_object_positions()
        ...
        change_in_left = left_wheel_speed_end - left_wheel_speed_start
        change_in_right = right_wheel_speed_end - right_wheel_speed_start

        change_left_per_step = change_in_left / self.args.steps_per_step
        change_right_per_step = change_in_right / self.args.steps_per_step

        joint_target_velocities_start = self.get_joint_speeds()
        joint_target_velocities_end = {}
        for key in joint_target_velocities:
            velocity_end = relative_to(joint_target_velocities[key],
                                       -getattr(self.args, 'max_joint_speed'),
                                       getattr(self.args, 'max_joint_speed'))
            joint_target_velocities[key] = velocity_end
            joint_target_velocities_end[key] = velocity_end

        change_in_joint_per_step = {}
        for key in joint_target_velocities:
            change = joint_target_velocities_end[key] - joint_target_velocities_start[key]
            change_in_joint_per_step[key] = change / self.args.steps_per_step

        joint_target_velocities_step = {
            step: {
                key: joint_target_velocities_start[key] + change_in_joint_per_step[key] * (step + 1)
                for key in joint_target_velocities
            } for step in range(self.args.steps_per_step)
        }

        for step in range(self.args.steps_per_step):
            left_wheel_step = left_wheel_speed_start + change_left_per_step * (step + 1)
            right_wheel_step = right_wheel_speed_start + change_right_per_step * (step + 1)
            self.set_wheel_speeds(left_wheel_step, right_wheel_step)

            joint_step = self.fix_joints(deepcopy(joint_target_velocities_step[step]))
            for key, value in joint_step.items():
                if joint_target_velocities_step[step][key] != value:
                    for s in joint_target_velocities_step:
                        joint_target_velocities_step[s][key] = value
            self.set_joint_target_velocities(joint_step)
