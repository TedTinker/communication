...
def make_reducer(data, labels, reducer_dict, args):
    for i, goal_part in enumerate(["task", "color", "shape"]):
        set_seed(42)
        scaler = StandardScaler()
        data_scaled = scaler.fit_transform(data)
        if(args.reducer_type == "pca"):
            reducer = PCA(n_components=1, random_state=42) # I might need to do something totally different for LDA?
            reducer.fit(data_scaled)
        if(args.reducer_type == "lda"):
            reducer = LDA(n_components=1)
            reducer.fit(data, labels[:,i])
        reducer_dict[args.epochs][goal_part] = {"scaler": scaler, "reducer" : reducer}
    return reducer_dict
...
def use_reducer(data, reducer_dict):
    reduced_dict = {}
    for goal_part in ["task", "color", "shape"]: 
        data_scaled = reducer_dict[goal_part]["scaler"].transform(data)
        reduced = reducer_dict[goal_part]["reducer"].transform(data_scaled)
        reduced_dict[goal_part] = reduced
    return(reduced_dict)
...
def plot_by_epoch_vs_epoch(reduced_dict, args):
    ...
    reduced_task  = reduced_dict["reduced"]["task"]
    reduced_color  = reduced_dict["reduced"]["color"]
    reduced_shape = reduced_dict["reduced"]["shape"]
    ...
    def plot_by_attribute(ax, reduced_dict_1, reduced_dict_2, classes, title): 
        ...
        for task, color, shape, x, y in zip(reduced_dict["tasks"], reduced_dict["colors"], reduced_dict["shapes"], reduced_dict_1, reduced_dict_2):
        ...
        for (task, color, shape), coords in grouped_points.items():
            ...
            if("shape" in classes):
                ...
                ax.add_artist(ab)
            if("task" in classes):
                ax.scatter(x, y, color=text_color_val, marker=f'${letter}$', alpha=1, s=s, edgecolor='none', zorder=2)
    ...
    plot_by_attribute(axes[0], reduced_task, reduced_color, ("task", "color"),  "Task and Color")
    plot_by_attribute(axes[1], reduced_task, reduced_shape, ("task", "shape"),  "Task and Shape")
    plot_by_attribute(axes[2], reduced_color, reduced_shape, ("color", "shape"), "Color and Shape")
