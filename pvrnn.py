...
class PVRNN(nn.Module):
    ...
    def bottom_to_top_step_prediction(self, prev_hidden_states, action):
        prev_hidden_states = prev_hidden_states.to(self.args.device)
        wheels_shoulders = action.wheels_shoulders.unsqueeze(1)  # Shape (episodes, 1, ...)
        voice_out = action.voice_out.unsqueeze(1)  # Shape (episodes, 1, ...)

        zp_inputs = torch.cat([prev_hidden_states, action.wheels_shoulders, action.voice_out], dim=-1)

        episodes, steps = zp_inputs.shape[0], zp_inputs.shape[1]
        zp_inputs = zp_inputs.reshape(episodes * steps, zp_inputs.shape[2])

        # Process zp for each modality using the prior networks
        def process_zp(zp_inputs, z_func):
            zp_mu = z_func.zp_mu(zp_inputs)
            zp_std = z_func.zp_std(zp_inputs)
            zp = sample(zp_mu, zp_std, self.args.device)
            return zp

        rgbd_zp = process_zp(zp_inputs, self.pvrnn_layer.rgbd_z)
        sensors_zp = process_zp(zp_inputs, self.pvrnn_layer.sensors_z)
        father_voice_zp = process_zp(zp_inputs, self.pvrnn_layer.father_voice_z)
        mother_voice_zp = process_zp(zp_inputs, self.pvrnn_layer.mother_voice_z)
        ...
        # Update hidden state using the MTRNN
        new_hidden_states_p = self.pvrnn_layer.mtrnn(mtrnn_inputs_p, prev_hidden_states)

        return new_hidden_states_p



    def multi_step_prediction(self, prev_hidden_states, actions):
        episodes, total_steps, hidden_size = prev_hidden_states.shape
        steps = total_steps - 1  # Since prev_hidden_states has shape (episodes, steps+1, hidden_size)

        predictions = []

        for step in range(steps):
            # Initialize hidden state at this step
            h = prev_hidden_states[:, step].unsqueeze(1)  # Shape (episodes, 1, hidden_size)

            # Determine the maximum number of steps ahead we can predict from this starting point
            max_steps_ahead = min(self.args.steps_ahead, total_steps - step - 1)

            # Initialize list to store predictions for this starting point
            step_predictions = []

            # For t in range of available steps ahead
            for t in range(max_steps_ahead):
                # Get the action at time step (step + t)
                action_wheels_shoulders = actions.wheels_shoulders[:, step + t]
                action_voice_out = actions.voice_out[:, step + t]
                action = Action(action_wheels_shoulders, action_voice_out)
